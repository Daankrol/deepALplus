...
NUM_QUERY = args_task["batch"]
NUM_INIT_LB = args_task["initial_pool"]
NUM_ROUND = int(args_task["quota"] / args_task["batch"])
...
print("GPU is available: {}. device: {}".format(use_cuda, device))
...
while iteration > 0:
    ...
    strategy = get_strategy(
        args_input.ALstrategy, dataset, net, args_input, args_task
    )  # load strategy

    start = datetime.datetime.now()

    # generate initial labeled pool
    dataset.initialize_labels(args_task["initial_pool"])
    ...
    for rd in range(1, NUM_ROUND + 1):
        ...
        if "CEALSampling" in args_input.ALstrategy:
            q_idxs, new_data = strategy.query(
                NUM_QUERY, rd, option=args_input.ALstrategy[13:]
            )
        else:
    ...
    if args_input.save_model:
        # save model
        timestamp = (
            re.sub("\.[0-9]*", "_", str(datetime.datetime.now()))
            .replace(" ", "_")
            .replace("-", "")
            .replace(":", "")
        )
...
if not os.path.exists(
    os.path.dirname(os.path.abspath("") + "/results/" + file_name_res_tot)
):
    os.makedirs(os.path.dirname(os.path.abspath("") + "/results/" + file_name_res_tot))
file_res_tot = open(
    os.path.join(os.path.abspath("") + "/results", "%s" % file_name_res_tot), "w"
)
...
file_res_tot.writelines(
    "number of unlabeled pool: {}".format(dataset.n_pool - NUM_INIT_LB) + "\n"
)
...
file_res_tot.writelines(
    "time of repeat experiments: {}".format(args_input.iteration) + "\n"
)

# result
for i in range(len(all_acc)):
    acc_m.append(get_aubc(args_task["quota"], NUM_QUERY, all_acc[i]))
...
file_res_tot.writelines(
    "mean acc: " + str(mean_acc) + ". std dev acc: " + str(stddev_acc) + "\n"
)
file_res_tot.writelines(
    "mean time: " + str(mean_time) + ". std dev acc: " + str(stddev_time) + "\n"
)
...
file_res = open(
    os.path.join(os.path.abspath("") + "/results", "%s" % file_name_res), "w"
)
...
for i in range(len(avg_acc)):
    tmp = (
        ...
        + str(NUM_INIT_LB + i * NUM_QUERY)
        + ", "
        + "accuracy is "
        + str(round(avg_acc[i], 4))
        + "."
        + "\n"
    )
